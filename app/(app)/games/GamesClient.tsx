...
export default function GamesClient({ userWords }: GamesClientProps) {
  ...
  const initMatchingGame = () => {
    // Take up to 4 words randomly
    const shuffled = [...userWords].sort(() => 0.5 - Math.random()).slice(0, 4);
  ...
  const initSpellingGame = () => {
    ...
    setTimeout(() => speakWord(userWords[0]?.term || ''), 500);
  };

  const handleCheckSpell = () => {
    const currentWord = userWords[currentWordIdx];
  ...
  const handleNextSpell = () => {
    if (currentWordIdx + 1 < Math.min(userWords.length, 5)) {
      const nextIdx = currentWordIdx + 1;
      setCurrentWordIdx(nextIdx);
      setSpellInput('');
      setSpellChecked(false);
      setTimeout(() => speakWord(userWords[nextIdx]?.term || ''), 500);
    } else {
      setGameOver(true);
  ...
  const initFillBlankGame = (idx: number) => {
    const currentWord = userWords[idx];
    if (!currentWord) return;
    ...
    // Generate random options containing the correct answer + 3 random other words
  ...
  const nextSpeedQuestion = () => {
    const randWord = userWords[Math.floor(Math.random() * userWords.length)];
    if (!randWord) return;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {GAMES.map((g) => (
              <div
                ...
                className={`bg-gradient-to-br ${g.color} rounded-2xl p-6 text-white shadow-md cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col justify-between`}
              >
                <div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4 text-sm font-bold text-slate-700 dark:text-slate-300">
              <span>Điểm: <span className="text-indigo-600 dark:text-indigo-400">{score}</span></span>
            ...
            <>
                ...
                <div className="max-w-md mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-lg">
                  <div className="text-center space-y-2">
                    <span className="text-xs font-bold text-indigo-650 bg-indigo-50 dark:bg-indigo-950/40 px-3 py-1 rounded-full uppercase tracking-wider">
                      Từ {currentWordIdx + 1} / {Math.min(userWords.length, 5)}
                    </span>
                  ...
                  <div className="flex justify-center py-4">
                    <button
                      onClick={() => speakWord(userWords[currentWordIdx]?.term || '')}
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">
                        Từ loại gợi ý: {userWords[currentWordIdx]?.partOfSpeech || 'n/a'}
                      </label>
                      <input
                        type="text"
                        value={spellInput}
                    ...
                    {spellChecked && (
                      <div className={`p-4 rounded-xl flex items-start gap-3 ${isCorrectSpell ? 'bg-emerald-50 text-emerald-800 dark:bg-emerald-950/20 dark:text-emerald-300' : 'bg-red-50 text-red-800 dark:bg-red-950/20 dark:text-red-300'}`}>
                        ...
                        <div>
                          ...
                          <p className="text-xs mt-0.5">{userWords[currentWordIdx]?.definitionVi || userWords[currentWordIdx]?.definition}</p>
                        </div>
                      </div>
                    )}
              ...
              {activeGame === 'fill_blank' && (
                <div className="max-w-xl mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-lg">
                  <div className="text-center space-y-2">
                    <span className="text-xs font-bold text-indigo-650 bg-indigo-50 dark:bg-indigo-950/40 px-3 py-1 rounded-full uppercase tracking-wider">
                      Câu {currentWordIdx + 1} / {Math.min(userWords.length, 5)}
                    </span>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl text-center space-y-2">
                    ...
                    <p className="text-base text-slate-800 dark:text-slate-100 font-medium italic">
                      &ldquo;
                      {userWords[currentWordIdx]?.exampleSentence
                        ?.replace(new RegExp(userWords[currentWordIdx]?.term, 'gi'), '_______') || '_______'}
                      &rdquo;
                    </p>
                    {/* Vietnamese translation helper */}
                    {userWords[currentWordIdx]?.exampleSentenceVi && (
                      ...
                      <button
                        ...
                        className={`p-3.5 rounded-xl border-2 font-bold text-sm transition-all cursor-pointer ${
                          selectedFillOption === opt
                            ? fillChecked
                              ? opt === userWords[currentWordIdx]?.term
                            ...
                            : fillChecked && opt === userWords[currentWordIdx]?.term
